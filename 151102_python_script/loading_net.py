...
def see(i):
   f=open('loading_net.dat','a')
   fn = get_fn(i)
   ds = yt.load(fn)
   ad = ds.all_data()
#   halo = ad.cut_region("obj['z']>0.2") and ad.cut_region("obj['z']<-0.2")
   halo1 = (ad.cut_region("obj['z']>0.2")).cut_region("obj['z-velocity']>0.")
   halo2 = (ad.cut_region("obj['z']<-0.2")).cut_region("obj['z-velocity']<0.")

   metal_inject=10**-3.9*204.4 # 204.4 * SFR density, in unit of Msun/yr/kpc^2
   energy_inject=0.00349* (10**-3.1) #case, in unit of erg/s/cm**2
   SF_density = 10**-3.9
